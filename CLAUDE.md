...
### 例外：オーナーがスレッドで直接動いたとき

上の「CMO経由」は**原則**であって、絶対ではない。
**オーナーがスレッドで直接アクション（メッセージ等）をしたときは例外扱いとする。**
CMO待ちだと間に合わないケースがあるため（2026-09-11 オーナー指示）。

- オーナーが直接出した指示は、掲示板の判断より**優先する**。
- その場でオーナーに聞き返してよい（カード形式で）。
- 掲示板の指示がオーナーの直接指示と食い違うときは、**実装せず、食い違いを掲示板に書いて止める。**
  黙って上書きしない。お金の決定が本人の知らないところで変わるため。
