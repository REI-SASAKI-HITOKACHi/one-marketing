...
>   依頼と確認が済むまでURLを変えない。
> - **`lp.one-hitter.jp` は使えない**（2026-09-12 リアライズが対応不可と回答）。
>   当面 `.netlify.app` のまま。LP専用ドメインの新規取得を進行中。
>   新ドメインを当てても `.netlify.app` のURLは残す（既存の登録を生かすため）。
>   **切り替えの実行は「外に出す」なので、必ず先にCMOへ一言入れる。**
