...
console.log('\n--- 連名の相手（代表）を決める ---');
{
  const ctx = makeContext({
    ...
    '代理店募集人マスタ': [
      ['代理店名', '氏名', '代表', '有効', '備考'],
      ['ひとりだけ代理店', '単独 一郎', false, true, ''],
      ['代表あり代理店', '甲野 一郎', false, true, ''],
      ['代表あり代理店', '乙野 二郎', true,  true, ''],
      ['代表あり代理店', '丙野 三郎', true,  true, '代表が2人。先に書いてあるほうを使う'],
      ['代表なし代理店', '丁野 四郎', false, true, ''],
      ['代表なし代理店', '戊野 五郎', false, true, ''],
      ['退職者だけ代理店', '退職 済', true, false, '']
    ]
  });
  const co = n => ctx.coAgentFor_(n);
  t('1人しかいなければその人',   co('ひとりだけ代理店'), '単独 一郎');
  t('複数なら代表に印の付いた人', co('代表あり代理店'), '乙野 二郎');
  t('代表が決まらなければ空',     co('代表なし代理店'), '');
  t('登録がなければ空',           co('登録なし代理店'), '');
  t('知らない代理店でも落ちない', co('そんな代理店はない'), '');
  t('代表の決まらない代理店を拾う', ctx.agenciesWithoutRepresentative_(), ['代表なし代理店']);
}
