...
for r in rows:
    ...
    if re.fullmatch(r'[ぁ-んァ-ヶー]', sei(g(r, '氏名'))):
        data.append(('保留', f'氏名の表記を確認してください（台帳:{g(r, "氏名")}）',
                     num, '')); continue
    keitou = g(r, '送信系統')
    # 流入経路が本舗系なのに送信系統が自社になっている行が119名ある（2026-09-11 顧客接点担当の指摘）。
    # 「本舗顧客には本舗として営業する」（2026-09-08 オーナー指示）に照らして、
    # どちらの名乗りで送るかをオーナーに確認するまで、未送信の行は保留にする。
    # 送信済みの行はそのまま（記録を書き換えない）。
    if (keitou == '自社' and keiro in HONPO_KEIRO and not g(r, '送信済み')):
        data.append(('保留', f'流入経路が「{keiro}」なのに送信系統が自社。'
                     'どちらの名乗りで送るかオーナーに確認中（2026-09-11）', num, ''))
        continue
    hon = honbun(r, keitou)
    if not hon:
