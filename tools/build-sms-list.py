...
ATAMA = ['優先', '顧客名', '電話番号', '▶SMSを開く', '送信済み',
         ...
         # 2026-09-19 追加。顧客接点担当が手で入れる2列。
         #   ここを見出しに入れていなかったため、作り直すと**並べ替えで行とズレていた**
         #   （A:Y だけ消していたので消えはしないが、別のお客様の行に付く）。
         #   いまは下の HOZON で、お名前をたよりに拾い直してから書き戻す。
         '返信内容（本文）', '返信の分類']

# ============ 作り直す前に、人が手で入れたものを拾っておく（2026-09-19） ============
#   このツールは台帳から作り直すので、**シートに手で入れたものは何もしないと消えるか、
#   並べ替えで別の人の行に付く。** 実際に 2026-09-19、返信6名の記録と、
#   名義が変わった2名の「保留」が、作り直しで飛ぶ状態になっていた。
#   拾うのは次の3つだけ。台帳から作れるものは拾わない（二重管理にしない）。
HOZON = {}
try:
    _ima = call(f'/{SS}/values/{TAB}!A5:AA2000').get('values', [])
    _atama = _ima[0] if _ima else []
    def _ix(na):
        return _atama.index(na) if na in _atama else -1
    _i_na, _i_ku, _i_ri = _ix('顧客名'), _ix('配信区分'), _ix('送らない理由')
    _i_hon, _i_bun = _ix('返信内容（本文）'), _ix('返信の分類')
    for _r in _ima[1:]:
        _r = list(_r) + [''] * 40
        _na = str(_r[_i_na]).strip() if _i_na >= 0 else ''
        if not _na:
            continue
        HOZON[_na] = {
            '配信区分': str(_r[_i_ku]).strip() if _i_ku >= 0 else '',
            '送らない理由': str(_r[_i_ri]).strip() if _i_ri >= 0 else '',
            '返信内容（本文）': str(_r[_i_hon]).strip() if _i_hon >= 0 else '',
            '返信の分類': str(_r[_i_bun]).strip() if _i_bun >= 0 else '',
        }
    print(f'いまのシートから拾いました: {len(HOZON)}行'
          f'（返信の記録 {sum(1 for v in HOZON.values() if v["返信内容（本文）"])}件／'
          f'手で保留 {sum(1 for v in HOZON.values() if v["配信区分"] == "保留")}件）')
except Exception as _e:
    print(f'⚠ いまのシートが読めませんでした（{_e}）。'
          f'**返信の記録と手の保留が消えます。止めてください。**')
    raise
...
for i, r in enumerate(rows):
    ...
    # ★2026-09-19：人が手で「保留」にした行は、作り直しても保留のままにする。
    #   名義が変わった方を止めた保留が、作り直しで送信可に戻ると 9/11 の事故が再発する。
    #   逆向き（機械が保留にしたものを送信可に戻す）は**しない**。安全側にだけ倒す。
    _te = HOZON.get(str(g(r, '氏名')).strip(), {})
    if _te.get('配信区分') == '保留' and kubun == '送信可':
        kubun = '保留'
        riyuu = _te.get('送らない理由') or '人が手で保留にした行です（作り直しでも解除しません）'
        hon = ''
    link = ''
    if kubun == '送信可':
        # ★=HYPERLINK() は使わない。
...
if os.environ.get('SMS_TEST_ROW') == '1':
    atarashii.insert(0, (test_gyou + [''] * len(ATAMA))[:len(ATAMA)])
...
# いったん広めに消してから書き直す
# ★AA まで消す。Y までにしていたため、Z・AA（返信の記録）が古い並びのまま残り、
#   並べ替え後の別のお客様の行に付いていた（2026-09-19 に判明）。
call(f'/{SS}/values/{TAB}!A1:AA2000:clear', 'POST', {})
