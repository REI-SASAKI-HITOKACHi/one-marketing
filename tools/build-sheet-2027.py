...
L_RYUNYU = [
    ...
    # 2026-09-26 追加：検索広告（2026-09-20 配信開始）。10/9 の判定で数える
    "広告",
    "その他",
]
...
def build_uriage(wb, m, plan_m, net_target):
    ...
    agg = [
        ...
        ("受注日",
         f'=IFERROR(SUMPRODUCT(({rng(Cc["受注日"])}<>"")*({rng(Cc["施工日付"])}<>"")'
         f'*({rng(Cc["施工日付"])}-{rng(Cc["受注日"])}))'
         f'/SUMPRODUCT(({rng(Cc["受注日"])}<>"")*({rng(Cc["施工日付"])}<>"")),0)',
         None, '"受注→施工 "0"日"'),
        ...
        ("郵便番号(-無し)",
         f'=SUMIFS({rng(Cc["売上（税込）"])},{rng(Cc["売上種類"])},"One Hitter")', None, '"自社 "#,##0'),
        ("住所",
         f'=SUMIFS({rng(Cc["売上（税込）"])},{rng(Cc["売上種類"])},"本舗")', None, '"本舗 "#,##0'),
    ...
    for r in range(U_FIRST, U_LAST + 1):
        ws.cell(row=r, column=U_IDX["受注日"]).number_format = DATE
...
def build_nenkan(wb, plan, plan_year):
    ...
    # --- 自社／本舗（2026-09-21 追加）
    # ブロック①〜⑤は流入経路で切っていて、自社と本舗が混ざる。
    # 売上種類でも見られるようにする（2026年は自社+26%・本舗−27%で中身が別物だった）。
    ws.cell(row=55, column=1, value="▼ 自社／本舗（売上種類で見る）").font = F_SECT
    row(56, "自社（One Hitter）売上", lambda m, i: U4(m, "郵便番号(-無し)"), YEN, "sum",
        "2026年は1〜9月で +26%。伸びているのはこちら", bold=True)
    row(57, "おそうじ本舗 売上", lambda m, i: U4(m, "住所"), YEN, "sum",
        "2026年は1〜9月で −27%。楽ラクーン・スケジュールマッチング・入札はここ")
    row(58, "自社の比率",
        lambda m, i: f'=IFERROR({get_column_letter(2+i)}56/{get_column_letter(2+i)}20,"")',
        PCT, '=IFERROR(N56/N20,"")', "自社売上 ÷ 売上実績")
...
def build_dashboard(wb, plan_year):
    ...
    alerts = [
        (24, "提携先 新規の成約 社数", "=COUNTIF('提携先_パイプライン'!$G$5:$G$104,\"成約\")", 5,
         "アクティブ10社→15社。1社増えると年50万円規模で効く"),
        (29, "自社（One Hitter）売上 累計", "='年間成績'!$N$56", None,
         "本舗の1円はロイヤリティで39銭出ていく。自社の1円のほうが残る"),
...
def build_teikei(wb):
    ...
    title(ws, "提携先パイプライン — アクティブ10社 → 15社（中立）",
          "平均単価100,128円（大型案件込み）／大型を除いた実力 49,130円／秋（9〜12月）の実単価 56,931円。"
          "LTV186,272円・リピート率45%。1社増えると年50万円規模で効きます。")
    label(ws, "A3", "新規の成約 社数", FILL_GOAL)
    ws["B3"] = '=COUNTIF($G$5:$G$104,"成約")'
    ws["B3"].font = Font(bold=True, size=14)
    ws["B3"].fill = FILL_GOAL
    label(ws, "C3", "目標", FILL_GOAL)
    ws["D3"] = 5
    ws["D3"].fill = FILL_GOAL
    ws["E3"] = ("アクティブ10社（2026-09-14 時点）を 保守12社／中立15社／攻め18社 へ。"
                "目標欄は新規の成約社数＝保守2社／中立5社／攻め8社。"
                "入口は施設ターゲットリスト291件（BREXAのような施設・法人の天カセ案件が最も現実的）。"
                "※不動産管理会社の空室清掃はやらない（オーナー決定 2026-09-14）")
    ws["E3"].font = F_NOTE
    ws.conditional_formatting.add(
    ...
    # 列名で持つ。列を足しても書式とプルダウンの位置がずれない
    cols = ["先方名", "業種", "担当者", "初回接触日", "最終接触日", "接触回数(累計)",
            "ステージ", "紹介件数(累計)", "売上(累計)", "次アクション", "次アクション期日",
            "担当", "メモ"]
    widths(ws, [26, 16, 14, 13, 13, 13, 14, 13, 14, 30, 14, 12, 36])
    header_row(ws, 4, cols)
    T = {n: i + 1 for i, n in enumerate(cols)}
    TC = {n: get_column_letter(i) for n, i in T.items()}
    for r in range(5, 105):
        ws.cell(row=r, column=T["初回接触日"]).number_format = DATE
        ws.cell(row=r, column=T["最終接触日"]).number_format = DATE
        ws.cell(row=r, column=T["接触回数(累計)"]).number_format = '0"回"'
        ws.cell(row=r, column=T["売上(累計)"]).number_format = YEN
        ws.cell(row=r, column=T["次アクション期日"]).number_format = DATE
    dv(ws, L_TEIKEI_STAGE, [f'{TC["ステージ"]}5:{TC["ステージ"]}104'])
    ws.freeze_panes = "A5"
    ws.auto_filter.ref = f'A4:{TC["メモ"]}104'
    return ws
...
def build_memo(wb):
    ...
    seed = [
        ...
        (date(2026, 9, 13), "年間成績",
         ...
         "CMO配下スレッド", "—", "—"),
        (date(2026, 9, 26), "選択肢マスター（流入経路）",
         "「広告」を追加し、④自社ネット新規の数え方（L_NET_NEW）にも入れた。"
         "検索広告が2026-09-20に配信を始めたのに、2027年版の選択肢に無く、④に数えられない状態だった（作り落とし）。"
         "予約ページ・GBP予約ボタンなど『入口の仕組み』は流入経路ではなく『流入元の識別子(?src=)』列で見る",
         "経営企画室", "—", "—"),
        (date(2026, 9, 21), "年間成績・ダッシュボード・◯月_売上顧客",
         "「自社／本舗」で見る行を追加（年間成績 56〜58行・ダッシュボード29行）。"
         "ブロック①〜⑤は流入経路で切っていて自社と本舗が混ざるため。"
         "2026年は1〜9月で自社+26%（¥8,627,308）・本舗−27%（¥3,729,583）と中身が別物だった。"
         "月次タブの4行目に『自社 ◯円』『本舗 ◯円』の自動集計を足してある（列の追加ではなく空きセルの利用）",
         "経営企画室", "—", "—"),
        (date(2026, 9, 17), "提携先_パイプライン",
         "①列を追加：最終接触日・接触回数(累計)。提携先1社に何回接触して1件受注できるかが、"
         "どこにも記録されていなかったため（2026年の台帳には接触の記録が無い）。"
         "②目標を「2社→4社」から「アクティブ10社→12/15/18社」に修正。2社は2026-09-14にオーナーが訂正した古い数字。"
         "③空室清掃の記載を削除（オーナー決定 2026-09-14「完全なる悪手」）。"
         "④ダッシュボードの参照列もE→Gへ追従（ステージ列が移動したため）",
         "経営企画室", "—", "—"),
        (date(2026, 9, 17), "◯月_売上顧客（12タブ）",
         "列を追加：受注日（施工日付の隣）。4行目に「受注→施工 平均◯日」が自動で出る。"
         "2026年の台帳は施工日付しか無く、受注が止まったのかリードタイムが短いだけなのかを"
         "区別できなかった。生成スクリプトから作り直し、追加の前後で集計値が変わらないことを機械で確認済み",
         "経営企画室", "—", "—"),
    ]
...
TOC = [
    ...
    ("提携先_パイプライン", "アクティブ10社→15社。最終接触日・接触回数・ステージ・次アクション", "接触のたび"),
...
SELFTEST_EXPECT = {
    ...
    # 4行目の自動集計は、列を足すと位置がずれる。列名から引く
    f"'1月_売上顧客'!{UC['売上種類']}4": 3,                # 自社件数
    f"'1月_売上顧客'!{UC['受注日']}4": 7.75,               # 平均リードタイム（16+5+5+5)/4 日
    f"'1月_売上顧客'!{UC['施工日付']}4": 1,                # 本舗件数
    f"'1月_売上顧客'!{UC['流入経路']}4": 4,                # 合計件数
    f"'1月_売上顧客'!{UC['流入元の識別子(?src=)']}4": 30000,  # 単価
    f"'1月_売上顧客'!{UC['TEL(-無し)']}4": 25000,          # 本舗単価
    f"'1月_売上顧客'!{UC['郵便番号(-無し)']}4": 95000,      # 自社売上（20000+30000+45000）
    f"'1月_売上顧客'!{UC['住所']}4": 25000,                # 本舗売上
    f"'1月_売上顧客'!{UC['売上（税込）']}4": 120000,        # 合計売上
    f"'1月_売上顧客'!{UC['備考(お客様の声)']}4": 2,         # ④自社ネット新規 件数（LP＋HP）
    f"'1月_売上顧客'!{UC['入金経路']}4": 50000,            # ④自社ネット新規 売上
    "'1月_売上顧客'!D2": 120000,
...
def selftest(path):
    ...
    # 列名で書く。列を足しても位置がずれない
    rows = [
        {"通し番号": 1, "売上種類": "One Hitter", "受注日": date(2026, 12, 20),
         "施工日付": date(2027, 1, 5), "流入経路": "LP", "流入元の識別子(?src=)": "lp_ga",
         "売上（税込）": 20000, "実施メニュー": "エアコン(ノーマル)", "施工担当": "和真"},
        {"通し番号": 2, "売上種類": "One Hitter", "受注日": date(2027, 1, 1),
         "施工日付": date(2027, 1, 6), "流入経路": "HP", "流入元の識別子(?src=)": "hp",
         "売上（税込）": 30000, "実施メニュー": "浴室", "施工担当": "和真"},
        {"通し番号": 3, "売上種類": "本舗", "受注日": date(2027, 1, 2),
         "施工日付": date(2027, 1, 7), "流入経路": "楽ラクーン", "流入元の識別子(?src=)": "",
         "売上（税込）": 25000, "実施メニュー": "換気扇", "施工担当": "協力業者A"},
        {"通し番号": 4, "売上種類": "One Hitter", "受注日": date(2027, 1, 3),
         "施工日付": date(2027, 1, 8), "流入経路": "リピート", "流入元の識別子(?src=)": "",
         "売上（税込）": 45000, "実施メニュー": "エアコン(ロボ)", "施工担当": "和真"},
    ]
    for i, r in enumerate(rows):
        for name, v in r.items():
            u.cell(row=U_FIRST + i, column=U_IDX[name], value=v)
