...
(function(){
  ...
  /* ── 流入経路の持ち回り ───────────────────────────────
     広告から来た人は LP に ?src=gads&cid=… &gclid=… で着地し、
     そこから予約フォーム（別ホスト）へ移る。URL に付いていなければ
     この端末に保存したものを使う。90日で消える。
     ★別ホストなので localStorage は共有されない。LP 側で
       予約フォームへのリンクにパラメータを付け直す必要がある（lp 担当）。 */
  var KEIRO_KEY = 'oh_keiro';
  var KEIRO_HI = 90 * 24 * 60 * 60 * 1000;
  ...
  function keiroHozon(){
    ...
    if (!Object.keys(mitsuketa).length) { return; }
    mitsuketa._t = Date.now();
    try { localStorage.setItem(KEIRO_KEY, JSON.stringify(mitsuketa)); } catch (e) {}
  }
  ...
  function keiro(k){
    ...
    if (q) { return q; }
    try {
      var h = JSON.parse(localStorage.getItem(KEIRO_KEY) || '{}');
      if (h._t && Date.now() - h._t > KEIRO_HI) { return ''; }
      return h[k] || '';
    } catch (e) { return ''; }
  }
