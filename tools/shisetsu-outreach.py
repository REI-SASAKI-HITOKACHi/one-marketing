...
―― 以下は、置いていただける場合の補足です ――
・読み物の末尾に、当社のクリーニング（{y['menu']}・税込。エアコンはお掃除機能の無いタイプの価格です。出張費はかかりません。駐車スペースが無い場合はコインパーキング代が実費になります）と、エアコン・洗濯槽・追い焚き配管の無料点検のご案内を載せています
...
async def fill_form(pg, url: str, text: str, email_from: str, subject: str, hop: int = 0) -> dict:
    ...
    #
    # 2026-09-18 修正：除外語の数だけで外していたら、**犬猫も扱う大きなペット店を誤って外した。**
    # 実測（ページ本文の出現回数）：
    #   エイチ・ツー（水槽の店）  除外語6種／犬猫の語 0回   → 対象外でよい
    #   小林昆虫                 除外語3種／犬猫の語 0回   → 対象外でよい
