...
function shomenRows(v){
  return [
    ...
    ['役務の対価（税込）', '<span class="v">' + v.price + '</span>' + (v.discount ? '<br>' + v.discount : '') + '<br>上記以外の費用（出張費・追加作業費）は請求しません。駐車スペースをご用意いただけない場合は、コインパーキング等の駐車場代を実費でご請求します（金額は事前にお伝えします）。'],
...
(function(){{
  ...
  function draw(){{
    ...
    h += '<p class="note">所要 '+mm.所要+'。出張費・追加作業費はありません。駐車スペースが無い場合はコインパーキング代を実費でいただきます（金額は事前にお伝えします）。</p>';
