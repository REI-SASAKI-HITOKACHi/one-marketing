...
#
# 2026-09-25 訂正（オーナー決定「B 総額2,600万を守る」）
#   旧値 5,390,000 は 2026年1〜8月の本舗 ¥3,691,083 を ×1.459（全社の季節性）した推計値で、
#   オーナー決定の「今年度と同じ数字」を推計値で読んでいた。
#   実績：1〜9月 ¥3,729,583（cmo の売上種類の割り直し／経営企画室が独立集計と突合して一致）
#         ＋ Q4 約16万（スケジュールマッチング・楽ラクーンは戻らない見込み）＝ 通年 約389万
#   総売上 2,600万 は動かさず、減った 150万 は自社で埋める（自社 2,061万 → 2,211万）。
#   本舗が減るとロイヤリティ（39%）も減るので、経常利益は 507万 → 566万 に上がる。
HONPO_URIAGE_NEN_EN = 3_890_000
...
# ---------------------------------------------------------------
# 2028〜2030年のシミュレーション（2027年の中立を達成した場合）
#
# 【考え方】2027年に自社で獲った新規が、翌年リピートで戻ってくる。それが積み上がる。
#   ロジックは2027年の計画から一切変えていない。変えたのは3つの前提だけ：
#     ・自社で獲った新規のリピート率（実測36%。docs/流入経路の分析.md）
#     ・自社ネット新規の月件数
#     ・アクティブな提携先の社数
#   本舗案件は3年とも据え置き（390万）。伸ばさない前提は変えていない。
# ---------------------------------------------------------------
CHOUKI_KEISU = {
    'リピート継続率': 0.90,      # 既存のリピート層の自然減を10%と置く
    'リピート単価倍率': 1.13,    # 2026年のリピート平均単価45,374円 ÷ 全体平均40,062円
    '本舗_万': 390,              # 据え置き
    'その他_万': 20,
    '資材費_円per件': 225,
    '外注_支払率': 0.65,
    '補助人件費率': 0.014,
    '変動経費率': 0.068,
    'ロイヤリティ_万': 270,      # 固定60万＋本舗売上539万×39%＝210万
    '固定経費_万': 192,
    '融資返済_万': 46,
    '和真_万': 600,
    '社員1人_万': 552,           # 月40万×12×社保1.15
    '1人あたり年間件数': 380,    # 2026年に和真さん1人で469件（本舗含む）をこなしている実績から
}

CHOUKI_AN = {
    # index 0 = 2027年（計画の中立）。1〜3 = 2028〜2030年
    '保守的': {'リピート率': 0.30, 'ネット新規_月件数': [12, 14, 16, 18],
               'ネット単価': [35000, 36000, 36000, 37000],
               '提携社数': [15, 16, 17, 18], '提携1社_万': [60, 60, 62, 62],
               '外注率': 0.15, '実効CPA': 7500, '平均単価': [40062, 40500, 41000, 41500]},
    '中':     {'リピート率': 0.36, 'ネット新規_月件数': [12, 18, 24, 28],
               'ネット単価': [35000, 37000, 38000, 39000],
               '提携社数': [15, 18, 21, 24], '提携1社_万': [60, 62, 65, 67],
               '外注率': 0.18, '実効CPA': 7000, '平均単価': [40062, 41000, 42000, 43000]},
    '良':     {'リピート率': 0.40, 'ネット新規_月件数': [12, 21, 29, 36],
               'ネット単価': [35000, 38000, 39000, 40000],
               '提携社数': [15, 19, 23, 27], '提携1社_万': [60, 64, 67, 69],
               '外注率': 0.20, '実効CPA': 6500, '平均単価': [40062, 41500, 43000, 44500]},
}

RIPEATO_2027_MAN = 850   # 2027年の① 自社リピート資産
# ※提携社数の index 0（2027年）は中立の目標15社。2026年の実績は10社
#   （2026-09-14 に株式会社テック山口を追加して 9→10社）


def chouki(an_mei):
    """2027年の中立を達成した前提で、2028〜2030年を出す。"""
    k, a = CHOUKI_KEISU, CHOUKI_AN[an_mei]
    net = [a['ネット新規_月件数'][i] * 12 * a['ネット単価'][i] / MAN for i in range(4)]
    ripeeto = RIPEATO_2027_MAN
    out = []
    g2027 = goukei(keisan('中立'))
    for i in range(4):
        if i == 0:
            # 2027年は計画（中立）の実値をそのまま使う。ここは推計しない
            out.append({'年': 2027, '売上_万': round(g2027['売上'] / MAN), '件数': g2027['件数'],
                        'リピート_万': RIPEATO_2027_MAN, '提携_万': 840, '本舗_万': k['本舗_万'],
                        'ネット_万': 500, '社員数': 2,
                        '協力業者_件': sum(AN['中立']['外注件数']),
                        '広告費_万': round(g2027['販管_広告費'] / MAN),
                        '原価_万': round(g2027['原価_計'] / MAN),
                        '販管費_万': round(g2027['販管_計'] / MAN),
                        '経常利益_万': round(g2027['経常利益'] / MAN)})
            continue
        ripeeto = ripeeto * k['リピート継続率'] + net[i - 1] * a['リピート率'] * k['リピート単価倍率']
        teikei = a['提携社数'][i] * a['提携1社_万'][i]
        uriage = ripeeto + teikei + k['本舗_万'] + net[i] + k['その他_万']
        kensuu = round(uriage * MAN / a['平均単価'][i])
        gaichu_kensuu = round(kensuu * a['外注率'])
        ninzuu = max(2, -(-(kensuu - gaichu_kensuu) // k['1人あたり年間件数']))
        koukoku = a['ネット新規_月件数'][i] * 12 * a['実効CPA'] / MAN
        genka = (kensuu * k['資材費_円per件'] / MAN
                 + uriage * a['外注率'] * k['外注_支払率']
                 + uriage * k['補助人件費率'])
        hankan = (k['固定経費_万'] + k['ロイヤリティ_万'] + koukoku
                  + k['和真_万'] + (ninzuu - 1) * k['社員1人_万']
                  + uriage * k['変動経費率'])
        keijou = uriage - genka - hankan - k['融資返済_万']
        out.append({'年': 2027 + i, '売上_万': round(uriage), '件数': kensuu,
                    'リピート_万': round(ripeeto), '提携_万': teikei, '本舗_万': k['本舗_万'],
                    'ネット_万': round(net[i]), '社員数': ninzuu, '協力業者_件': gaichu_kensuu,
                    '広告費_万': round(koukoku), '原価_万': round(genka),
                    '販管費_万': round(hankan), '経常利益_万': round(keijou)})
    return out
