 ...
 *
 * ■ 認証
 * スクリプトプロパティ `API_TOKEN` と、リクエストの token が一致しないと何もしない。
...
/**
 * 書類番号の採番。見積書・請求書とも `YYYYMMDD-nn` 形式（例 20260915-01）。
 *
 * 2026-09、過去の請求書に合わせてこの形式に決めた（CMO判断 20260915-01-cmo）。
 * 過去の番号は `00116-03` `10118-01` のような5桁始まりで、規則が読み取れず
 * 「最大番号の次」を機械的に決められなかった。日付ベースなら発行の事実だけで
 * 一意に決まり、5桁始まりの旧番号とは桁数で区別できるので過去分に触らずに済む。
 *
 * 旧IDの `EST-` / `INV-` 接頭辞は付けない。接頭辞付きの既存行は
 * 今日の日付で始まらないので走査に引っかからず、採番に影響しない。
 *
 * 同じ日に100件を超えたら3桁になる（`20260915-100`）。桁が伸びるだけで破綻しない。
 */
function generateDocumentId_(sheet, idHeader, sheetLabel) {
  const prefix = formatDate_(new Date(), 'yyyyMMdd') + '-';
  const info = getHeaderInfo_(sheet);
  const col = info.map[idHeader];
  if (!col) throw new Error(sheetLabel + 'に ' + idHeader + ' ヘッダーがありません。');
  ...
  if (lastRow > info.headerRow) {
    ...
    ids.forEach(function (r) {
      ...
      // 枝番は数字だけ。`20260915-01-控` のような派生は採番対象にしない
...
function generateEstimateId_(sheet) {
  return generateDocumentId_(sheet, 'estimate_id', '見積データシート');
}
