...
function adminDeployAll() {
  ...
  out.push('');
  out.push('※ 画面の上に「通常見積 / WEB経由見積」のタブがあります。既定は通常見積。');
  out.push('');
  out.push('■ 通常見積タブ（改修前と同じ。金額は1円も変わりません）');
  out.push('   ・繁忙期加算 ¥3,300 は税抜として加算（従来どおり）');
  out.push('   ・同時施工価格・ネット申込特典は使わない');
  out.push('   ・自動割引は auto_discount_enabled = FALSE。現場に周知してから TRUE にしてください');
  out.push('');
  out.push('■ WEB経由見積タブ（公開中の予約フォームと同じ金額）');
  out.push('   ・2箇所以上は同時施工価格（浴室・キッチン・レンジフードなら1箇所目以外 ¥13,800）');
  out.push('   ・繁忙期加算 ¥3,300 は税込として扱う');
  out.push('   ・ネット申込特典 −¥2,200 は2箇所以上で同時施工割引が付かないときだけ自動で付く');
  out.push('   ・自動割引は auto_discount_enabled_web = TRUE（フォームと金額を揃えるため）');
  out.push('');
  out.push('※ どちらで作ったかは見積レコードの「受注経路」列に残ります。'); 
...
function upsertMissingSettings_(masterSs) {
  ...
  const defaults = [
    ...
    ['auto_discount_enabled_web', 'TRUE', 'WEB経由見積で早期予約割引・複数台割引を使うか', '予約フォームと金額を揃えるためON。通常見積は上の auto_discount_enabled（FALSE）が効く'],
...
function adminDiagnose() {
  ...
  add('');
  add('■ 自動割引の設定');
  add('  auto_discount_enabled　　： ' + (parseBooleanLoose_(settings['自動割引有効']) ? 'TRUE（通常見積に自動で割引が載ります）' : 'FALSE（通常見積に自動割引は載りません）'));
  add('  auto_discount_enabled_web： ' + (parseBooleanLoose_(settings['自動割引有効_WEB経由']) ? 'TRUE（WEB経由見積に自動で割引が載ります）' : 'FALSE（WEB経由見積に自動割引は載りません）'));
  ...
  if (!parseBooleanLoose_(settings['自動割引有効_WEB経由'])) {
    ...
    add('    金額を揃えるなら auto_discount_enabled_web を TRUE にしてください。');
  }
