...
| # | 仕組み | 効く範囲 |
|---|---|---|
| 1 | `requestId` の記録 | **30分以内**の再送。同じIDなら1件目の番号を返すだけで新規作成しない |
| 2 | 見積レコードの `invoice_id` | **期限なし**。既に請求書がある見積に2通目を作らせない |

**請求書は2段とも効きます。** 30分を過ぎた再送でも、2の側で止まります
（`この見積には既に請求書 ... が作成されています` というエラーになります）。
作り直したいときだけ `payload.allowRecreate: true` を明示してください。

**見積は1しかありません。** 30分を過ぎてから同じ `requestId` で再送すると、見積が2件できます。
実務上は再送が30分以内に起きるので足りていますが、**この差は承知しておいてください。**
