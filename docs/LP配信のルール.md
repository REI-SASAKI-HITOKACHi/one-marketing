...
## 確認は1コマンドで流せます（2026-09-15 追加）

道具が5つに分かれていて、順番と引数を覚えていないと使えませんでした。まとめてあります。
...
**終了コードが 0 なら配信してよい、1 なら止める。** 中で流しているもの：
...
**他スレッドの持ち物の未解決（読本・点検のダークモード）は、配信の可否に含めず、
参考として最後に出します。**含めると、こちらの配信がいつまでも○になりません。

> **3項目は正規表現で見ています。** 下の文書には `color-scheme: light` と
> 空白つきで書いてありますが、**ビルドすると最小化されて `color-scheme:light` になります。**
> 文字列の完全一致で見ると、正しいのに落ちます（実際に落としました）。
