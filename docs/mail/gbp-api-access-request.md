...
| 進捗 | 画面 | やること |
|---|---|---|
| **5%** | アカウントの確認 | 「onehitter.her@gmail.com でログインしています」と出る。「確認」で次へ（別アカウントへの切替も可） |
| **12%** | お客様のビジネスを選択してください | 一覧に **ワンヒッター㈱／東京都江戸川区北葛西5-14-…／ステータス「確認済み」** が出る（**条件を満たしている**）。選んで「次へ進む」 |
| **33%** | プロジェクトおよび会社情報を入力します | 下の表のとおり入力。**ここから先はオーナーが入力する**（社外への申請のため） |
| 33%以降 | 未確認 | 画面に従う |
...
| 会社のウェブサイト＊ | `https://one-hitter.jp/` |
| この API アクセス フォームをどのようにして知りましたか＊ | **Google の公式ドキュメント（developers.google.com/my-business の前提条件ページ）** |
| （もう1項目あり・画面下に隠れていて未確認） | 画面で確認する |

> ブラウザ担当は 33% の入力で自動判定に止められた（社外への申請フォームのため）。**何も送信していない。**
> **ブラウザ担当は画面を出して1項目ずつ読み上げる役に回り、入力と送信はオーナーが行う。**
