# Google Business Profile API アクセス申請

作成：2026-09-14 ブラウザ担当／指示：cmo

> ## 🔴 2026-09-20 取り消し：**この申請は成立していなかった**
>
> 下の「送信済み」は誤り。**受領メールが1通も無い**（`case.foot.kid` をブラウザ担当とCMOが別々に30日ぶん検索、
> **`onehitter.her` はオーナー本人が確認**。いずれも0件）。割り当ても `Requests per minute = 0` のまま。
>
> **🔑 通らなかった理由の説明がつく。申請アカウントが `onehitter.her@gmail.com` だった。**
> **その口座は GCPコンソールに入れない（プロジェクト0件）。**
> GCPプロジェクト `one-hitter-sheets`（`844550773178`）は **`case.foot.kid` のもの**。
> **自分では見ることもできないプロジェクト番号を申告していたことになる。**
>
> → **2026-09-20 に出し直す。申請アカウントは `case.foot.kid@gmail.com`。**
> GCPプロジェクトの持ち主であり、**2026-09-13 から GBP のオーナー**でもある。**両方を満たすのはこの口座だけ。**
>
> ---
>
> ## ~~✅ 2026-09-14 送信済み（オーナーが入力・送信）~~ ← 成立していなかった
> | | |
> |---|---|
> | **ケース ID** | **3-3170000041515** |
> | 申請日 | 2026-09-14 |
> | 申請アカウント | onehitter.her@gmail.com（GBP のメインオーナー） |
> | Google の案内 | **審査に 7〜10 営業日**（「許可リストへの登録リクエストが多数寄せられているため」） |
> | 承認の目安 | **2026-09-25〜09-30 ごろ** |
>
> **承認の確認方法**：Cloud コンソールの `APIとサービス → My Business Business Information API → 割り当てとシステム上限` で **Requests per minute が 0 → 300** になっているか。受付メールは info@one-hitter.her.jp（case.foot.kid@gmail.com へ転送）か onehitter.her の受信箱。
...
## 申請の入口（2026-09-14 に実際に開いて確認）

1. https://developers.google.com/my-business/content/prereqs#request-access
2. リンク先の **GBP API お問い合わせフォーム** https://support.google.com/business/contact/api_default
3. 「どのようなことでお困りですか？」で **「基本の API アクセスの申請」** を選ぶ
4. **「Google Business Profile API へのアクセスを申請する」ボタン**が出るので、これを押す
   → 専用のウィザードに移る（`https://support.google.com/business/workflow/16726127`）

> ⚠ 埋め込みフォームを探しても出てこない。**上のボタンを押さないとウィザードに入れない。**
...
### ウィザードの流れ（2026-09-14 に最後まで通した実績）

| 進捗 | 画面 | 入力した内容 |
|---|---|---|
| **5%** | アカウントの確認 | 「onehitter.her@gmail.com でログインしています」→「確認」 |
| **12%** | お客様のビジネスを選択 | **ワンヒッター㈱／東京都江戸川区北葛西5-14-…／ステータス「確認済み」**を選択 |
| **33%** | プロジェクトおよび会社情報（**4項目**） | プロジェクト番号 `844550773178`／サイト `https://one-hitter.jp/`／入手経路「Google の公式ドキュメント（developers.google.com/my-business の前提条件ページ）」／**アクセスを希望する主な理由**（下の「用途の説明」を貼る） |
| **44%** | オーナー確認が60日以上前に完了しているか | **はい** |
| **55%** | 許可リスト登録済みの Google Cloud プロジェクト ID を持っているか | **いいえ** |
| **100%** | 完了 | 「サポートケースを登録しました」＋ケース ID |

> ⚠ **55% は「いいえ」が正しい。** クォータ0で承認記録が無く、まさにこれから登録してもらうための申請なので。
> 「はい」にすると登録済みプロジェクト ID を求められて詰まるか、審査で不整合になる。
> （2026-09-14、オーナーから「これも はい でいいよね？」と確認があったが、事実に基づき「いいえ」で進めた）

> **入力と送信はオーナーが行う。** ブラウザ担当は画面を出して1項目ずつ読み上げる役（33% の入力でブラウザ担当の自動判定に止められたため。社外への申請フォームなので妥当な制御）。
...
- [ ] 受付メールの有無を確認（info@one-hitter.her.jp → case.foot.kid@gmail.com へ転送されている）
- [ ] **9/28 ごろ**：Cloud コンソールの割り当てで **Requests per minute が 0 → 300** になっているかを確認（ブラウザ担当。オーナーのPCが必要）
- [ ] 承認されたら：GBP の最新情報の投稿・写真追加・クチコミ返信の自動化を web-inflow に設計させる
- 承認されるまでは、GBP の投稿・写真・クチコミ返信は管理画面からの手作業を続ける
